...
pub fn strip_transparency() {
    ...
    let mut raw_pixels: Vec<(u32, u32, &image::Rgba<u8>)> = workable_image.enumerate_pixels().collect();

    for (_, y, Rgba { data: [_, _, _, alpha] }) in &raw_pixels {
        if top_row.0 == false && alpha > &0 {
            top_row = (true, *y);
            break;
        }
    }
    ...
    for x in 0..columns {
        for y in 0..rows {
            ...
            let (_, _, Rgba { data: [_, _, _, alpha] }) = raw_pixels[index];
            if left_column.0 == false && alpha > &0 {
                left_column = (true, x);
                break;
            }
        }

        if left_column.0 == true {
            break;
        }
    }
    ...
    for x in (0..columns).rev() {
        for y in 0..rows {
            let index: usize = ((y * columns) + x) as usize;
            let (_, _, Rgba { data: [_, _, _, alpha] }) = raw_pixels[index];
            if right_column.0 == false && alpha > &0 {
                right_column = (true, x);
                break;
            }
        }

        if right_column.0 == true {
            break;
        }
    }
    ...
    for (_, y, Rgba { data: [_, _, _, alpha] }) in &raw_pixels {
        if bottom_row.0 == false && alpha > &0 {
            bottom_row = (true, *y);
            break;
        }
    }

    println!("top {:?}, bottom {:?}, left {:?}, right {:?}", top_row, bottom_row, left_column, right_column);
    ...
    bounded_image.save(output_file_path).expect("Expected to save sub image");
}
